...
Trace each group against what the target intends — the spec where there is one, otherwise the behavior the target clearly relies on: a behavior-neutral refactor changes nothing but citations; a changed asset or preserved behavior means the target is stale on something it meant to keep; a changed integration point means re-checking whether the target's adaptation still reproduces the reference contract; changed packaging usually means nothing, unless it now carries behavior the capability needs; behavior added, removed, or reverted is a scope decision. For prompt-driven features, use the lenses in [behavioral-design.md](../distillation-spec/references/behavioral-design.md).
